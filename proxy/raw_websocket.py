...
TLS_VERIFY_STRICT = "strict"
TLS_VERIFY_PERMISSIVE = "permissive"


def _make_ssl_context(mode: str) -> ssl.SSLContext:
    ctx = ssl.create_default_context()
    if mode == TLS_VERIFY_PERMISSIVE:
        ctx.check_hostname = False
        ctx.verify_mode = ssl.CERT_NONE
        return ctx
    if mode == TLS_VERIFY_STRICT:
        return ctx
    raise ValueError(f"Unknown TLS verification mode: {mode}")


_ssl_ctx_strict = _make_ssl_context(TLS_VERIFY_STRICT)
_ssl_ctx_permissive = _make_ssl_context(TLS_VERIFY_PERMISSIVE)


def get_ssl_context(mode: str) -> ssl.SSLContext:
    if mode == TLS_VERIFY_STRICT:
        return _ssl_ctx_strict
    if mode == TLS_VERIFY_PERMISSIVE:
        return _ssl_ctx_permissive
    raise ValueError(f"Unknown TLS verification mode: {mode}")
...
def _xor_mask(data: bytes, mask: bytes) -> bytes:
    ...
    return (int.from_bytes(data, "big") ^ int.from_bytes(mask_rep, "big")).to_bytes(
        n, "big"
    )
...
class RawWebSocket:
    ...
    @staticmethod
    async def connect(
        host: str, domain: str, timeout: float = 10.0, tls_mode: str = TLS_VERIFY_STRICT
    ) -> "RawWebSocket":
        ssl_ctx = get_ssl_context(tls_mode)
        ...
        while not self._closed:
            ...
            if opcode == self.OP_CLOSE:
                ...
                try:
                    self.writer.write(
                        self._build_frame(
                            self.OP_CLOSE, payload[:2] if payload else b"", mask=True
                        )
                    )
                    await self.writer.drain()
                except Exception:
